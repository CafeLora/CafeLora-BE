...
namespace Aplicacion.UmbralesMinColor
{
    public class Nuevo
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, Ejecuta>
        {
            ...
            {
                //    var umbralExists = await _context.UmbralMinTemperatura.Where(pr => pr.Id == request.Id).FirstOrDefaultAsync();

                //  var lastUmbral = _context.UmbralMinTemperatura.OrderByDescending(ev => ev.Id).FirstOrDefault().Id;
                //var auxUmbral = 0;
                ...
                var umbral = new UmbralMinColor
                {
                    // Id=lastUmbral+1,
                    Ultimo = request.Ultimo,
                };

                _context.UmbralMinColor.Add(umbral);
                // }
